  ...
  import { Apple, Boxes, Ruler, Utensils, ChevronLeft, Menu, ChevronDown } from 'lucide-react';
    ...
    return <Collapsible.Root open ={open} onOpenChange={setOpen}>
        ...
        >
          {items.map((item)=> (
            ...
            asChild
            key={item.href}>
       <Link className={`flex items-center rounded-md px-5 py-1 transtion-all ${
        pathname === item.href
          ...
          <div className="bg-background fixed z-10 flex h-13 w-screen items-center justify-between border px-2">
              <Collapsible.Root className="h-full" open={open} onOpenChange={setOpen}>
                ...
                <Collapsible.Root className = "fixed top-0 left-0 z-20 h-dvh" open={open} onOpenChange={setOpen}>  
                  <Collapsible.Content forceMount>
                      <div className={`bg-background fixed top-0 left-0 h-screen w-64 border p-4 transition-transform duration-300 z-20 ${open ? "translate-x-0" : "-translate-x-full"}`}>
